locals {
  ...
  len_endpoint_subnets    = max(length(var.endpoint_subnets), length(var.endpoint_subnet_ipv6_prefixes))
  len_gwlb_subnets        = max(length(var.gwlb_subnets), length(var.gwlb_subnet_ipv6_prefixes))
  len_loadbalancer_subnets = max(length(var.loadbalancer_subnets), length(var.loadbalancer_subnet_ipv6_prefixes))
  ...
  max_subnet_length = max(
    ...
    local.len_endpoint_subnets,
    local.len_gwlb_subnets,
    local.len_loadbalancer_subnets
  )
  ...
  tags = merge(
    {
      Name = try(
        ...
        format("${var.name}-${var.public_subnet_suffix}-%s", element(split("", var.azs[count.index]), length(var.azs[count.index]) -1))
      )
    },
    var.tags,
  ...
  tags = merge(
    {
      Name = try(
        ...
        format("${var.name}-${var.private_subnet_suffix}-%s", element(split("", var.azs[count.index]), length(var.azs[count.index]) -1))
      )
    },
    var.tags,
  ...
  tags = merge(
    {
      Name = try(
        ...
        format("${var.name}-${var.redshift_subnet_suffix}-%s", element(split("", var.azs[count.index]), length(var.azs[count.index]) -1))
      )
    },
    var.tags,
  ...
  tags = merge(
    {
      Name = try(
        ...
        format("${var.name}-${var.elasticache_subnet_suffix}-%s", element(split("", var.azs[count.index]), length(var.azs[count.index]) -1))
      )
    },
    var.tags,
  ...
  tags = merge(
    {
      Name = try(
        ...
        format("${var.name}-${var.intra_subnet_suffix}-%s", element(split("", var.azs[count.index]), length(var.azs[count.index]) -1))
      )
    },
    var.tags,
  ...
  tags = merge(
    {
      Name = try(
        ...
        format("${var.name}-${var.outpost_subnet_suffix}-%s", element(split("", var.azs[count.index]), length(var.azs[count.index]) -1))
      )
    },
    var.tags,
